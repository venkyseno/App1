              ...
              <Link key={service.id} to={`/other-services/${service.id}`}>
                <Card className="h-full hover:-translate-y-0.5">
                  <div className="flex gap-4">
                    {service.imageUrl ? (
                      <img src={service.imageUrl} alt={service.name} className="h-24 w-28 rounded-lg object-cover" />
                    ) : (
                      <div className="h-24 w-28 rounded-lg bg-gray-100 flex items-center justify-center text-xs text-gray-500">No image</div>
                    )}
